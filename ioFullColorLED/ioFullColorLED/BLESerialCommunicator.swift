...
public class BLESerialCommunicator: NSObject {
    ...
    public class var sharedInstance : BLESerialCommunicator {
        struct Static {
...
*/
    ...
    func writeNum(num : UInt8){
        
        var rx : CBCharacteristic? = localDevice?.getCharacteristic(self.UUID_VSP_SERVICE, CharacteristicUUID: self.UUID_RX)
        var data : NSData = NSData(bytes: [num], length: 1)
        println("send:\(data)");
        localDevice?.writeWithoutResponse(rx!, value: data)
    }
    */
    public func write(R : UInt8 , G : UInt8 , B : UInt8){
        
        var rx : CBCharacteristic? = localDevice?.getCharacteristic(self.UUID_VSP_SERVICE, CharacteristicUUID: self.UUID_RX)
        var data : NSData = NSData(bytes: [0x55, R , G , B], length: 4)
        println("send:\(data)");
        if var wnrappedRX = rx
        {
            localDevice?.writeWithoutResponse(wnrappedRX, value: data)
        }
    }
